...
taliseInit_t talInit = {
	...
	.tx =
	{
		.txProfile =
		{
			...
			.txInputRate_kHz = 245760,            /* Rx IQ data rate in kHz */
		...
		.txChannels = TAL_TXOFF,                            /* The desired Tx channels to enable during initialization */
	...
	.jesd204Settings =
	{
		...
		.framerB =
		{
			...
			.K = 32,                        /* number of frames in a multiframe (default=32), F*K must be a multiple of 4. (F=2*M/numberOfLanes) */
			.F = 1,                            /* F (number of bytes per frame) */
			.Np = 16,                            /* Np (converter sample resolution) */
