...
void RecvCallback(void *arg, struct udp_pcb *tpcb,
	...
	if((recv_icd_header.SRC_CODE == 0xE1U) && (recv_icd_header.DEST_CODE == 0x51U)){
		...
		if(recv_icd_header.CMD_CODE == 0x0010U){				//Set Center Frequency
			...
			*((uint64_t *)&reply_buf_udp[8]) = (uint64_t)(DPU_STATUS.CenterFreq);
			...
			//RCV Packet
			(void)memcpy(send_packet->payload, reply_buf_udp, p->len + 1);
			...
			if(BIT_STATUS.BIT_SET == 0U){
				...
				if(reply_buf_udp[8] == 1U){			//PBIT
					...
					//RCV Packet
					(void)memcpy(send_packet->payload, reply_buf_udp, p->len + 18);
					...
					//RCV Packet
					(void)memcpy(send_packet->payload, reply_buf_udp, p->len + 12);
		...
		/////////////////////// RF_CTRL //////////////////////////
			...
			if(reply_buf_udp[8] == SET){
				...
				//RCV Packet
				(void)memcpy(send_packet->payload, reply_buf_udp, p->len + 10);
...
int TransferData(void)
{
	...
	if(DataReady == 1U){
		while(true){
			...
			(void)memcpy(send_packet->payload, AddrSpecCurHeader, send_size);
